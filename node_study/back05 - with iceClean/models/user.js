const Sequelize = require("sequelize");

class User extends Sequelize.Model {
  // 컬럼 정의하는 함수
  static initiate(sequelize) {
    User.init(
      {
        name: {
          type: Sequelize.STRING(20), // varchar(20)
          allowNull: false, // NOT NULL 제약조건
          unique: true, // 유니크 제약조건
        },
        age: {
          type: Sequelize.INTEGER.UNSIGNED, // 양수만 허용
          allowNull: false, // NOT NULL 허용하지 않음
        },
        married: {
          type: Sequelize.BOOLEAN,
          allowNull: false,
        },
        comment: {
          type: Sequelize.TEXT, // 긴 문자열 허용
          allowNull: true,
        },
        create_at: {
          type: Sequelize.DATE, // 날짜 타입
          allowNull: false,
          defaultValue: Sequelize.NOW, // 현재 시간으로 기본값 설정
        },
      },
      {
        sequelize,
        timestamps: false, // createdAt, updatedAt 자동 생성 방지
        underscored: false, // true로 설정하면 created_at, updated_at으로 컬럼명이 변경됨
        modelName: "User", // 모델 이름
        tableName: "users", // 실제 테이블 이름
        paranoid: false, // true로 설정하면 deletedAt 컬럼이 추가되어 soft delete 기능 활성화
        charset: "utf8", // 문자 인코딩
        collate: "utf8_general_ci", // 문자 정렬 방식
      }
    );
  }

  // User 테이블과 Comment 테이블 관계 맵핑 함수
  static associate(db) {
    db.User.hasMany(db.Comment,{foreignKey: 'commenter', sourceKey: 'id'}); // User 모델과 Comment 모델의 관계 설정
  }
}

module.exports = User;
