...
router.post('/complete', async (req, res) => {
  ...
  try {
    // res_no 예약번호, momo 간단한 완료글자, files 배열로 파일에 내용이 문자열인 base64
    ...
    if (files && files.length > 0) {
      for (let i = 0; i < files.length; i++) {
        const file = files[i];
        // 파일명은 "clean-현재시간-랜덤숫자 9자리.확장자명" 으로 들어온다.
        const fileName = `clean-${Date.now()}-${Math.round(Math.random() * 1E9)}.${file.type.split('/')[1]}`;

        // Base64 데이터를 Buffer로 변환
        // 문자열을 binary(2진수) 0, 1 값으로 변경
        // 2가지 파일 타입: 문자열타입 파일, 0과 1로 구성된 binary 파일로 구성
    ...
    // 청소기사 테이블 수정, 완료 데이터를 데이터베이스에 저장
