  ...
  if (data && data.length > 0) {
    row = `
      <div class="view">
        ...
        <p><strong>예약금 : </strong>${
          data[0].deposit ? data[0].deposit + "만 원" : "미정"
        }</p>
        <p><strong>예약상태 : </strong>${
          data[0].state == 0 ? "예약대기" : "예약완료"
        }
      ...
      <div class="button-container">
        <button class="update-btn" onclick="Resupdate(${
          data[0].res_no
        })">수정</button>
...
async function Resupdate(reservationId) {
  ...
  if (error) {
    ...
    Swal.fire({
      icon: "error",
      text: "예약 데이터를 가져오는 중 오류가 발생했습니다.",
    });
    return;
  }
  ...
  const basicAddr = postcode
    ? addrParts.slice(1, -1).join(" ").trim()
    : addrParts.slice(0, -1).join(" ").trim();
  ...
  $Step02.innerHTML = `
    <div class="view">
      <form id="updateForm">
        <p><strong>이름 : </strong><input id="updateName" value="${
          data.name
        }"></p>
        <p><strong>연락처 : </strong><input id="updateTel" value="${
          data.tel
        }"></p>
        <p><strong>email : </strong><input id="updateEmail" value="${
          data.email
        }"></p>
        ...
        <p><strong>날짜 : </strong><input type="date" id="updateDate" value="${data.date.slice(
          0,
          10
        )}" min="${formatDate(today)}" max="${formatDate(maxDate)}"></p>
        <p><strong>시간 : </strong>
          <select id="updateTime">
            <option value="오전 10시 ~ 오후 1시" ${
              data.time === "오전 10시 ~ 오후 1시" ? "selected" : ""
            }>오전 10시 ~ 오후 1시</option>
            <option value="오후 2시 ~ 오후 5시" ${
              data.time === "오후 2시 ~ 오후 5시" ? "selected" : ""
            }>오후 2시 ~ 오후 5시</option>
            <option value="오후 4시 ~ 오후 7시" ${
              data.time === "오후 4시 ~ 오후 7시" ? "selected" : ""
            }>오후 4시 ~ 오후 7시</option>
            <option value="오후 6시 ~ 오후 9시" ${
              data.time === "오후 6시 ~ 오후 9시" ? "selected" : ""
            }>오후 6시 ~ 오후 9시</option>
          </select>
        </p>
        <p><strong>제빙기 모델명 : </strong><input id="updateModel" value="${
          data.model
        }"></p>
        <p><strong>제빙기 용량 : </strong>
          <select id="updateCapacity" onchange="updateDeposit()">
            <option value="20~50kg" ${
              data.capacity === "20~50kg" ? "selected" : ""
            }>20~50kg</option>
            <option value="50~100kg" ${
              data.capacity === "50~100kg" ? "selected" : ""
            }>50~100kg</option>
            <option value="100kg 이상" ${
              data.capacity === "100kg 이상" ? "selected" : ""
            }>100kg 이상</option>
          </select>
        </p>
        <p><strong>선택 서비스 : </strong>
          <select id="updateService">
            <option value="청소" ${
              data.service === "청소" ? "selected" : ""
            }>청소</option>
            <option value="수리" ${
              data.service === "수리" ? "selected" : ""
            }>수리</option>
          </select>
        </p>
        <p><strong>서비스 주기 : </strong>
          <select id="updateCycle">
            <option value="이번 한 번만" ${
              data.cycle === "이번 한 번만" ? "selected" : ""
            }>이번 한 번만</option>
            <option value="한 달에 한 번" ${
              data.cycle === "한 달에 한 번" ? "selected" : ""
            }>한 달에 한 번</option>
          </select>
        </p>
        <p><strong>추가 서비스 선택 : </strong>
          <select id="updateAdd">
            <option value="심화 청소" ${
              data.add === "심화 청소" ? "selected" : ""
            }>심화 청소</option>
            <option value="물탱크 청소" ${
              data.add === "물탱크 청소" ? "selected" : ""
            }>물탱크 청소</option>
            <option value="필터 교체" ${
              data.add === "필터 교체" ? "selected" : ""
            }>필터 교체</option>
            <option value="" ${!data.add ? "selected" : ""}>없음</option>
          </select>
        </p>
        <p><strong>특별 요청사항 : </strong><textarea id="updateRemark">${
          data.remark || ""
        }</textarea></p>
        <p><strong>예약금 : </strong><span id="updateDeposit">${
          data.deposit ? data.deposit + "만 원" : "미정"
        }</span></p>
        <p><strong>예약상태 : </strong>${
          data.state == 0 ? "예약대기" : "예약완료"
        }</p>
      </form>
    </div>
    <div class="button-container">
      <button class="update-btn" onclick="saveUpdate(${
        data.res_no
      })">저장</button>
...
async function saveUpdate(reservationId) {
  ...
  const updatedData = {
    ...
    addr: `${document.getElementById("updatePostcode").value},
        ${document
          .getElementById("updateAddr")
          .value.trim()
          .replaceAll(",", "")},
        ${document
          .getElementById("updateDetailAddr")
          .value.trim()
          .replaceAll(",", "")}`,
    ...
    deposit:
      document.getElementById("updateDeposit").textContent === "미정"
        ? null
        : parseInt(
            document
              .getElementById("updateDeposit")
              .textContent.replace("만 원", "")
          ),
  };
