...
PrivacyOk.addEventListener("click", async function () {
  ...
  // axios는 axios.post, axios.get 등으로 사용
  // fetch는 fetch(url, { method: 'POST', body: JSON.stringify(data), headers: { 'Content-Type': 'application/json' } })
  const rest = await fetch("http://localhost:3000/res/save", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify(reservationData), // resservationData를 JSON 문자열로 변환하여 전송
  });
  const receviedData = await rest.text(); // 응답을 텍스트로 변환
  console.log(rest);
  console.log(receviedData);

  // await 내용 back05에 넣기
  // console.log(data);
  if (receviedData === "success") {
    Swal.fire({
      title: "예약성공!",
      icon: "success",
      draggable: true,
